...
import { textSearch,createVetor } from "./vector/index.js";
//同步读取
...
export async function readDocToText() {
    ...
    for (let i = 0; i < dirInfo.length; i++) {
        ...
        const fileText = await readFileToText(path)
...
export async function splitDoc(docText) {
    const spliter = new RecursiveCharacterTextSplitter({
        chunkSize:50,//商业项目推荐100-500
        chunkOverlap:20,//和上一段最多重叠多少
        separators:['\n\n','\n','。',',','，'," ",'但是','.'],//分隔符优先级，越靠前优先级越高
    })
...
export async function searchByQuestion(qtest) {
    return await textSearch(qtest)
}

export async function createRAGContext(text) {
    let ragContext = fs.readFileSync('./ragContext.md').toString();
    const searchArr = await searchByQuestion(text)
    let allStr = ''
    ...
    ragContext = ragContext.replace('${text}',allStr)
    return ragContext
}